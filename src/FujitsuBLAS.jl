...
function __init__()
    if "LD_LIBRARY_PATH" in keys(ENV)
        if isfile(_libdummy)
            ...
            dlopen(_libdummy)
            libs = filter(contains("libfjlapackexsve"), dllist())
            libfjlapackexsve = isone(length(libs)) ? only(libs) : ""
        else
            ...
            libfjlapackexsve = find_library("libfjlapackexsve.so", library_path)
        end
        isempty(libfjlapackexsve) && return Cint(0)
        return BLAS.lbt_forward(libfjlapackexsve; clear=true)
    end
    return Cint(0)
end
